...
If the issues config `enabled` is `false`, tell the user the issues creation is not enabled and use `AskQuestion` for instructions. Do not proceed.
...
- Use `AskQuestion` to ask the user if the tag list is correct.
...
Use `AskQuestion` to confirm that the array is correct and allow the user to add or remove objects.
...
For each command, use `AskQuestion` to ask the user if the issue is correct.

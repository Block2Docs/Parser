...
describe( 'JsFileParser', () => {
	...
	it( 'parseFile extracts docblock tags', () => {
		...
		const paramTags = greet.docblock.tags.filter(
			( t ) => t.name === 'param'
		);
	...
	it( 'parseDirectory returns all files', () => {
		...
		assert.ok(
			'SampleClass.js' in result,
			'should contain SampleClass.js'
		);
	} );

	it( 'parseDirectory returns empty object for empty directory', () => {
		const tmpDir = fs.mkdtempSync(
			path.join( os.tmpdir(), 'block2docs_test_' )
		);
